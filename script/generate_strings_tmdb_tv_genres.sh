...
for lang in $LANG
do
 curl -s https://api.themoviedb.org/3/genre/tv/list\?api_key=${API}\&language=${lang} | jq . | grep \"name\": | grep -v null | sed 's/^.*"name": "\([^"]*\)"/\1/g' > result-$lang
done
...
# en master of all
curl -s https://api.themoviedb.org/3/genre/tv/list\?api_key=${API}\&language=en | jq . | sed 's/^.*"id": //g' | sed 's/^.*"name": "\([^"]*\)"/\1/g' | sed "/[]{}[]/d" | sed 'N;s/,\n/|/' > result
